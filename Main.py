...
main = tkinter.Tk()
main.title("Sentiment Analysis of Customer Product Reviews Using Machine Learning") #designing main screen
...
def Preprocessing():
    ...
    for i in range(len(train)):
        ...
        review = train.get_value(i,1,takeable = True)
    ...
    df = pd.DataFrame(tfidf, columns=vector.get_feature_names())
...
def upload():
    global filename
    global filename
    filename = filedialog.askopenfilename(initialdir = "Amazon_Reviews_dataset")
    text.delete('1.0', END)
...
def predict():
    global vector
    testfile = filedialog.askopenfilename(initialdir = "Amazon_Reviews_dataset")
...
font = ('times', 16, 'bold')
title = Label(main, text='Sentiment Analysis of Customer Product Reviews Using Machine Learning')
